...
if gender == 1:
    death_data = death_data_male
    survival_data = survival_data_male
    max_age = 109
    pgender = "男性"
elif gender == 2:
    death_data = death_data_female
    survival_data = survival_data_female
    max_age = 113
    pgender = "女性"

while True:
    input_line = input("保険金額を入力してください")
    try:
        insurance = int(input_line)
        break
    except ValueError:
        continue
...
v = 1 / (1 + rate)
for i in range(term):
    l_sum += survival_data[j] * (v ** i)
    d_sum += death_data[j] * (v ** (i + 1/2))
    j += 1

Fee = insurance * d_sum / l_sum
print(f"性別:{pgender}")
print(f"保険金額：{insurance}円 年齢：{age}歳 保険期間：{term}年 利率：{rate:.6f}")
print(f"保険料：{'{:.0f}'.format(Fee)}円")
